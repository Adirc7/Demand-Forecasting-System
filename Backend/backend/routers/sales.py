...
async def delete_sale(sale_id: str, user=Depends(require_role(['sales_manager']))):
    db = get_db()
    ...
    if sale and 'sku' in sale:
        ...
        try:
            ...
            # Sync stock to inventory collection
            prod = prod_ref.get().to_dict()
            if prod:
                db.collection('inventory').document(sale['sku']).set({
                    'current_stock': prod.get('current_stock', 0),
            ...
            print(f"Failed to revert stock for deleted sale: {e}")

    # 3. Wipe globally cached data sets
    invalidate_cache('sales', 'products', 'inventory', 'products_active')
